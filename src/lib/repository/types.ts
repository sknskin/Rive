...
export interface AtomicSessionWrite {
  session: ReadingSession;
  progressMode: "always" | "if-newer";
  markAsRead?: boolean;
...
export interface ReadingRepository {
  ...
  // 세션과 서재 진행 상태를 한 트랜잭션으로 저장한다. 호출자가 만든 id를 재사용해 재시도도 멱등적이다.
  // Persist the session and shelf progress in one transaction. A caller-owned id makes retries idempotent.
  saveSessionAtomic(input: AtomicSessionWrite): Promise<ReadingSession>;
  // 오입력 정정 수단 — 세션 수정/삭제와 책 완전 제거 (BACKLOG P0-B)
  // Correction tools — session edit/delete and full book removal (BACKLOG P0-B)
  updateSessionAndRecompute(
    id: string,
    bookId: string,
    patch: Partial<Omit<ReadingSession, "id" | "bookId" | "createdAt">>,
  ): Promise<void>;
  deleteSessionAndRecompute(id: string, bookId: string): Promise<void>;
